from engines.impulse_engine import ImpulseEngine
import json

class ImpulseEnginesCollection(object):
    def __new__(cls):
        if not hasattr(cls, 'instance'):
            cls.instance = super(ImpulseEnginesCollection, cls).__new__(cls)
        return cls.instance
    def __init__(self):
        if hasattr(self, 'loaded'):
            ...
            self.engine_file_name = "data/engines_impulse.json"
            self.load_data()        

    def add_engine(self, engine):
        if not type(engine) == ImpulseEngine:
            print("Not a valid Impulse Engine Object")
            return        
    ...
    def load_data(self):
        ...
        print("Parsing Impulse Engine file: " + self.engine_file_name + "...")
        try:
            ...
            for key in data:                                               # since the data is a dictionary
                eng = ImpulseEngine()                                             # for each entry
        ...
        self.loaded = 1
    def save_data(self):
        print("Saving Impulse Engine Data: " + self.engine_file_name)
